  ...
  async function execute(command) {
    ...
    const run = async () => {
      ...
      if (name === '/init' || name === '/reinit') {
        ...
        if (created) { emit(`Mirror: ${created.mirror}${created.backup ? `\nBackup: ${created.backup}` : ''}`); if (name === '/init' && stdin.isTTY && stdout.isTTY && !process.env.CI && await confirm(t('Open advanced configuration now?', 'Buka konfigurasi lanjutan sekarang?'))) postConfigure = true; }
        return {};
      }
      if (name === '/status') {
...
export async function main(argv = process.argv.slice(2)) {
  ...
  if (ctx.mode === 'original' && configuredState?.config?.autoSync?.enabled && configuredState.config.autoSync.enabled && !argv.includes('--plain')) {
    const watcher = spawn(process.execPath, [process.argv[1], '--watch'], { cwd: ctx.original, detached: false, stdio: 'ignore', env: { ...process.env, AIMP_WATCHER_PARENT_PID: String(process.pid) } }); watcher.unref();
  }

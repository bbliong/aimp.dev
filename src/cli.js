...
export async function main(argv = process.argv.slice(2)) {
  ...
  if (argv.includes('--watch')) {
    ...
    try { while (!controller.signal.aborted) { const current = await loadState(ctx.original); for (const pair of Object.values(current?.pairs || {})) { const request = path.join(pair.mirror, '.aimp', 'sync-request.json'); if (!await fs.stat(request).catch(() => null)) continue; const raw = JSON.parse(await fs.readFile(request, 'utf8')); if (raw.type !== 'sync-to-original' || raw.branch !== pair.branch || raw.batchId !== pair.batch) continue; await fs.writeFile(path.join(pair.mirror, '.aimp', 'sync-notification'), `Sync request found for ${pair.branch} at ${new Date().toISOString()}\n`, { mode: 0o600 }); const session = createSession(ctx, { signal: controller.signal, output: console.log, prompt: async () => 'y' }); try { await session.execute('/sync'); } catch (error) { console.error(`Auto-sync paused: ${error.message}`); } finally { await fs.rm(request, { force: true }); } } await new Promise(resolve => setTimeout(resolve, current?.config?.autoSync?.intervalMs || 2000)); } } finally { process.off('SIGINT', stop); } return;
  }
